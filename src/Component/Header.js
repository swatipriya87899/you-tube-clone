...
const Header = () => {
  ...
  return (
    <div className="h-20">
      <div className={"grid grid-cols-12 p-4 items-center fixed bg-white"+showMenu?"bg-gray-500 bg-opacity-25":""}>
        ...
        <div className="col-span-9 flex justify-center">
          <div className="flex">
            <div>
              ...
              {/* suggestion box */}
              {showSuggestion && (
                <div className="flex flex-col w-96 bg-white absolute top-20 shadow-lg rounded-md">
                  {suggestion.map((e) => (
                    <div className="p-3 hover:bg-gray-200 flex items-center">
                      <AiOutlineSearch />
                      <div className="ml-4 -mt-0 font-semibold">{e}</div>
                    </div>
                  ))}
                </div>
              )}
